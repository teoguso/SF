...
tfft_step= float(invar_dict['tfft_step'])
en_resol=float(invar_dict['en_resol'])
    ...
    if int(dict_c['calc_exp']) == 1:
       ...
       #print(str(" Starting time (elaps, cpu): %10.6e %10.6e"% (elaps1, cpu1)))
	    ...
	    import time
            ...
            print (" ### Calculation of exponential A(\omega)_TOC11..  ")
            toten, toc11_tot=calc_toc11(dict_c,tfft_step,minkpt,maxkpt,minband,maxband,en,enmin,enmax,ims,eqp,encut,en_resol)
            ...
            for i in xrange(len(toten)):
                outfile.write("%7.4f %15.10e\n"% (toten[i],toc11_tot[i])) # Dump string representations of arrays
            outfile.close()
            print (" A(\omega)_TOC11 written in", outname)
            plt.plot(toten,toc11_tot,label="ftot_toc11");
