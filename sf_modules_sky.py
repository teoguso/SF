...
def read_invar(infile='invar.in'):
    ...
    List of variables with default values:
     ...
     'calc_toc11': 0 or 1, calculate the toc11 spectra.
     'encut': 0,1,2 etc, the number of elements around w=0 that are removed in the frequence list in the integration to avoid divergency at w=0 
     'tfft_step': 0.01, the time step in the FT.
     'en_resol': 0.01, the energy resolution of ImSigam interplation and final toc11 spectral function. for the moment 0.01 is safe for including w=0
    """
    var_defaults = { 
            'sigmafile': None,   
    	    'calc_toc11':0,
...
def integ_w(x,ShiftIms,NewEn,tImag): # pay attention of NewEn==0!!!!!
        return 1.0/math.pi*ShiftIms[x]*(cmath.exp(-(NewEn[x])*tImag)-1.0)*(1.0/(NewEn[x]**2))

def calc_toc11(vardct,tfft_step,minkpt,maxkpt,minband,maxband,en,enmin,enmax,imskb,eqp,encut,en_resol):
        import time
        import numpy as np;
        import math, cmath
        import pyfftw;
	...
	wtk = np.array(vardct['wtk'])
    #	pdos = np.array(pdos)
        dt=tfft_step #convergent parameter    
        en_cut=encut
        toc11_tot=0
	en_list=[]
	for i in xrange(len(en)):
		en_list.append(round(en[i],2))
        for ik in xrange(nkpt):
                ikeff=minkpt+ik-1
                print ("kpoint = %02d" % (ikeff+1))
                for ib in xrange(0,nband):
                        ibeff = minband+ib-1
                        interpims = interp1d(en_list, imskb[ik,ib], kind = 'linear', axis = -1)
                        eqp[ik,ib]=round(eqp[ik,ib],2)
                        NewEn_min=-15. #en[0]-eqp[ik,ib] #it is difficult to decide this value when we have valence and core. it is enough to put it around 2*wp
                        print ("minum energy is:", NewEn_min)
                        if en_list[-1]<-eqp[ik,ib]:
                                NewEn_max=2.
                        else:
                                NewEn_max=-eqp[ik,ib]#  -eqp is only valid when we have only valence, the -eqp of core is too large
                        print ("maximu energy in two digits is" ,NewEn_max)
                        NewEn=np.arange(NewEn_min,NewEn_max+newdx,newdx)
                        NewEn_size=NewEn.size
                        NewIms=interpims(NewEn)
                        ShiftEn=np.arange(NewEn_min+eqp[ik,ib],NewEn_max+eqp[ik,ib]+newdx,newdx)
                        ShiftIms=interpims(ShiftEn) #ImSigma(w+egw)
                        ...
                        outfilekb.close()
                        tfft_max=0 # for electron removal 
                        tfft_min=-2*math.pi/newdx # for electron removal        
                        enfft_min=-2*math.pi/dt
                        enfft_max=0
                        denfft=newdx #give the energy resolution in the final spectral function that is the same as the inperplated ImSigma
                        after_fft_En=np.arange(enfft_min,-2*enfft_min,denfft)
                        #enshift=after_fft_En[-50]
                        # to do: think about how to put this energy shift such that the FT of exp(-i*(enshift)*Imt) is for sure a delta peak. 
                        #print "the universal shift energy is:", enshift
                        t_list=[]
                        gt_list=[]
                        for t in np.arange(tfft_min,tfft_max+dt,dt):
                                tImag=t*1j
                                t_list.append(t)
                                ct=0
                                for en in np.arange(0,NewEn_size-1,1):
                                        if abs(NewEn[en])<1e-5:
                                                en1=np.arange(0,en-en_cut,1)
                                                en2=np.arange(en+en_cut+1,NewEn_size-1,1)
                                                for i in np.concatenate((en1,en2), axis=0):
                                                        Areai=0.5*newdx*(integ_w(i,ShiftIms,NewEn,tImag)+integ_w(i+1,ShiftIms,NewEn,tImag))
                                                        ct+=Areai
                                #gt=(1.j)*cmath.exp(-(enshift)*tImag)*cmath.exp(ct)
                                gt=(1.j)*cmath.exp(ct)
                                gt_list.append(gt)
                        tsize=len(t_list)
                        print ("IFFT of ")
                        ...
                        fft_in = pyfftw.empty_aligned(tsize, dtype='complex128')
                        fft_out = pyfftw.empty_aligned(tsize, dtype='complex128')
                        ifft_object = pyfftw.FFTW(fft_in, fft_out,direction='FFTW_BACKWARD')
                        gw=ifft_object(gt_list)*(abs(tfft_min))
                        Imgw=(1./math.pi)*gw.imag*wtk[ikeff]
                        Imgw=np.concatenate((Imgw,Imgw),axis=0)
                        toc11_tot+=Imgw
                        print ("IFFT done!")
                        #elaps2 = time.time() - elaps1 - e0
                        #elaps2 = time.time() - elaps1 - e0
                        #cpu2 = time.clock() - cpu1 - c0
                        #print str(" Used time (elaps, cpu): %10.6e %10.6e"% (elaps2, cpu2))
                        outnamekb = "TOC11-k"+str("%02d"%(ikeff+1))+"-b"+str("%02d"%(ibeff+1))+".dat"
                        outfilekb = open(outnamekb,'w')
                        toc11_en=[]
                        toc11_en2=[]
                        totsp_list=[]
                        ...
                        for i in xrange(tsize*2) :
                                if after_fft_En[i]>=-20 and after_fft_En[i]<=5:
                                        tmplist.append(i)
                                        toc11_en.append(after_fft_En[i]+eqp[ik,ib])#+eqp[ik,ib]-enshift)
                                        toc11_sp.append(Imgw[i])
                                        totsp_list.append(toc11_tot[i])
                        for i in xrange(len(tmplist)):
                                        outfilekb.write("%8.4f %12.8e \n" % (toc11_en[i], toc11_sp[i]))
                        outfilekb.close()
        return toc11_en, totsp_list
